...
def plan(frame):
    """Populate the given frame with widgets and logic."""
    import customtkinter as ctk

    # Reuse the existing logic and widgets, but attach them to the provided frame
    global app_list_TB, quest_list_TB, tabBox

    # Textbox for app list
    app_list_TB = ctk.CTkTextbox(frame, width=1080, height=360)
    # app_list_TB.grid(row=0, column=0, columnspan=2)

    # Time and app options
    time = [">1 hour", ">2 hours", '>3 hours']
    temp_quest_time = time[0]
    app_list = get_all_app_list()
    tab_list = ["Any Tab", "Youtube", "Reddit", "Instagram", "Facebook"]
    ...
    # Example widgets added to the frame
    label = ctk.CTkLabel(master=frame, text="Welcome to Tab 2!")
    # label.pack(padx=20, pady=20)

    button = ctk.CTkButton(master=frame, text="Run", command=lambda: update_loop)
